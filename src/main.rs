...
fn main() -> Result<()> {
    ...
    let mut reader: Box<dyn Read> = if !matches.get_one::<String>("infile").is_none() {
        Box::new(BufReader::new(File::open(matches.get_one::<String>("infile").unwrap())?))
    } else {
        Box::new(io::stdin())
    };

    let mut writer: Box<dyn Write> = if !matches.get_one::<String>("outfile").is_none() {
    ...
    loop {
        ...
        if let Err(e) = writer.write_all(&buffer[..num_read]) { // [..x] -> [0..x] -> [..] all elements
